<?php
namespace GoogleCrawlers;

use GoogleCrawlers\Constant;

use GoogleCrawlers\DomeDone;

class SearchEngine extends Constant
{
    protected $searchUrl = '';
    ...
    public function get_curls($searchQuery)
    {
        try {
            $search_qeury = str_replace(" ","+", $searchQuery);
            $url = $this->searchUrl.'/search?q='.$search_qeury.'&num='.$this->resultLimit.'&start=1';
            $options = array(
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_FOLLOWLOCATION => true,
                CURLOPT_AUTOREFERER => true,
                CURLOPT_CONNECTTIMEOUT => 520,
                CURLOPT_TIMEOUT => 520,
    ...
    public function setEngine($searchUrl)
    {
        $this->searchUrl = $searchUrl;
    }
    public function search($searchQueries, $limit = null)
    {
        if($limit != null ){
        ...
        if(is_array($searchQueries)){
            return $this->domMManipulation($searchQueries);
        }
    ...
    public function domMManipulation($searchQueries)
    {
        $result = [];
        foreach($searchQueries as $searchQuery){
            $getHtml = $this->get_curls($searchQuery, $this->resultLimit);
            $html = $this->load($getHtml);

            foreach ($html->find('div[id=main] div.uEierd') as $key => $uEierd) {
                /**
                 * get links, title and discription
                 */
                $promoted = true;
                $link = $uEierd->find('a', 0)->href;
                $title = $uEierd->find('a', 0)->find('span[dir=ltr]', 0)->innertext;
                $discription = $uEierd->find('div[class=w1C3Le]', 0)->plaintext;
            ...
            foreach ($html->find('div[id=main] div.xpd') as $ZINbbc) {
                if (isset($ZINbbc->find('a', 0)->href) && isset($ZINbbc->find('a h3 span[dir=ltr]', 0)->innertext)) {

                    $promoted = false;
                    $link = $ZINbbc->find('a', 0)->href;
                    $title = $ZINbbc->find('a h3 span[dir=ltr]', 0)->plaintext;
                    $discription = '';
                    $discripts = $ZINbbc->find('div[class=kCrYT] span[dir=ltr]');

                    foreach ($discripts as $discript) {
                        $discription .= $discript->plaintext;
                    }
        ...
        return array_slice($result, 0, $this->resultLimit * count($searchQueries));
    }
